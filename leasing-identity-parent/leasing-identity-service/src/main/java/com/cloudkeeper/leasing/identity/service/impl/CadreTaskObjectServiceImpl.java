...
public class CadreTaskObjectServiceImpl extends BaseServiceImpl<CadreTaskObject> implements CadreTaskObjectService {
    ...
    public CadreTaskObject updateStatusByTaskIdAndObjectId(String taskId, String objectId) {
        ...
        // 更新镇进度
        String sql = "select count(case WHEN vc.state = '4' THEN 1 ELSE NULL END) as finish, count(case WHEN vc.state != '4' THEN 1 ELSE NULL END) as unfinish, count(*) as total FROM village_cadres vc WHERE vc.parentDistrictId = '" + objectId + "'";
        ...
        // 更新总进度
        String totalSql = "select count(case WHEN vc.state = '4' THEN 1 ELSE NULL END) as finish, count(case WHEN vc.state != '4' THEN 1 ELSE NULL END) as unfinish, count(*) as total FROM village_cadres vc WHERE vc.parentDistrictId like '01%'";
